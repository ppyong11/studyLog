...
- **DB ERD**
    ...
    Timer 테이블: 타이머 사용 기록 저장 테이블. 계획 설정 시 계획의 카테고리와 타이머에 등록하려는 카테고리가 일치해야 함.      Notification 테이블과 1:N 관계<br>
...
- **메인 화면 진입 시 로그인 유무에 따른 처리, 로그인, 토큰 재발급 Sequence Diagram**
      ...
      - 로그인 로직<br>
        a. id, pw에 해당하는 유저가 있을 시, jwt 생성 & redis에 refresh token 저장 & 쿠키에 담아 반환<br>
...
- **계획 및 타이머 등록**
  <img width="1000" height="956" alt="계획타이머등록_차트" src="https://github.com/user-attachments/assets/fd626d6c-a44f-4cc8-96d9-afdd8d239fe7" /></br>
  계획을 등록하면 타이머에 설정해 계획별 시간 관리가 수월해지고, 일간/주간/월간 리포트와 날짜별 달성률을 확인할 수 있습니다. <br>
  5분마다 스케쥴러를 통한 동기화 및 수동 동기화와 리셋 처리가 가능하고 동기화 시 경과 시간 갱신과 계획 완료 여부 처리를 진행합니다. 
